...
// ✅ Red Bull logo from repo (GitHub Pages)
...
// Ergast constructorId for Red Bull
...
// (Keeps your existing “only if exists” behavior out of the workflow complexity.)
...
async function buildJson() {
  ...
  // ✅ Your forced driver numbers / pinned lineup
  const driversBase = [
  ...
  try {
    ...
    if (ctorRow) {
      teamStanding = {
        ...
        wins: ctorRow.wins ?? "-",
        originalTeam: ctorRow?.Constructor?.name ?? "Red Bull",
      };
    }
    ...
    for (const d of drivers) {
      ...
      if (match) {
        ...
        // Optional: show the constructor name Ergast has them under
  ...
  return {
    ...
    redbull: {
      team: "Red Bull",
      teamLogoPng: getTeamLogoUrl(REDBULL_LOGO_FILE),
      teamStanding,
    },
    lastRace,
    drivers,
