...
// ✅ Mercedes logo from repo (GitHub Pages)
...
// Ergast constructorId for Mercedes
const ERGAST_CONSTRUCTOR_ID = "mercedes";
...
async function buildJson() {
  ...
  // ✅ Mercedes drivers (pinned lineup)
  // NOTE: Ergast familyName matching expects lastName to match "Antonelli" even if you display "Kimi Antonelli"
  const driversBase = [
    { firstName: "George", lastName: "Russell", code: "RUS", driverNumber: 63 },
    { firstName: "Andrea Kimi", lastName: "Antonelli", code: "ANT", driverNumber: 12 },
  ];
  ...
  try {
    ...
    if (ctorRow) {
      teamStanding = {
        ...
        wins: ctorRow.wins ?? "-",
        originalTeam: ctorRow?.Constructor?.name ?? "Mercedes",
      };
    }
  ...
  return {
    ...
    meta: {
      ...
      note:
        "Before the first race (or if data is unavailable), outputs '-' placeholders. After races, fills positions/points/wins from current standings. Positions formatted as P1, P2, etc.",
    },
    mercedes: {
      team: "Mercedes",
      teamLogoPng: getTeamLogoUrl(MERCEDES_LOGO_FILE),
      teamStanding,
    },
    lastRace,
    drivers,
