...
// ✅ Mercedes logo from your repo (update filename if needed)
...
// Mercedes naming can vary; try a few
...
// Mercedes constructorId in Ergast is typically "mercedes"
...
// ---------- OpenF1: get current Mercedes drivers (NO FALLBACK DRIVERS) ----------
...
async function getMercedesDriversFromOpenF1() {
...
async function buildJson() {
  ...
  const of1 = await getMercedesDriversFromOpenF1();
  ...
  try {
    ...
    // Team row
    const mercedesCtor = constructorStandings.find(
    ...
    if (mercedesCtor) {
      teamStanding = {
        team: "Mercedes",
        position: fmtPos(mercedesCtor.position),
        points: mercedesCtor.points ?? "-",
        wins: mercedesCtor.wins ?? "-",
        originalTeam: mercedesCtor?.Constructor?.name ?? "Mercedes",
        constructorId: ERGAST_CONSTRUCTOR_ID,
      };
    }
