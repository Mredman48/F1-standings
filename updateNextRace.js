...
// Your GitHub Pages base URL (update if your repo name/user changes)
const PAGES_BASE = "https://mredman48.github.io/F1-standings";

// Track map source repo (SVG -> we render to PNG)
const CIRCUIT_REPO_OWNER = "julesr0y";
const CIRCUIT_REPO_NAME = "f1-circuits-svg";
const CIRCUIT_REPO_REF = "main";

// Prefer black outline first
const TRACK_STYLES = ["black-outline", "black", "white-outline", "white"];

// Folder where PNGs are written (must be committed by workflow)
const TRACKMAP_DIR = "trackmaps";

const UA = "f1-standings-bot/1.0 (GitHub Actions)";

// Optional: emergency overrides for SVG filename if matching picks the wrong circuit
// Example: { match: "monaco", file: "monaco-1.svg", style: "black-outline" }
const FILE_OVERRIDES = [
  // { match: "monaco", file: "monaco-1.svg", style: "black-outline" },
];

// Optional: emergency overrides for country ISO2 if you ever need it
...
// 24-hour time (as you requested before)
...
// More robust: last comma-separated token treated as country
...
// Extract GP name from summary like "Australian GP - Race"
...
async function getCountryCodeAndFlag(countryName, gpName) {
  // manual override by gpName keyword
  ...
  // REST Countries lookup for ISO2
...
async function findBestTrackSvgFile({ gpName, city, country }) {
  ...
  for (const style of TRACK_STYLES) {
    ...
    if (top?.score >= 10) break; // good enough
  }
...
async function updateNextRace() {
  ...
  // Track map (PNG) and flag (PNG/SVG)
  ...
  const out = {
    ...
    grandPrix: {
      name: gpName,
      city,
      country: flag?.found ? flag.country : country,
    ...
    notes:
      "Use flag.pngUrl for the country flag (PNG) and trackMap.pngUrl for the circuit map (PNG). Local date/time strings are rendered for America/Edmonton.",
  };
